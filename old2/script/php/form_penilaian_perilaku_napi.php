...
$(document).ready(function(){
	$('#reset').click(function(){ 
		$('#divForm').load('<?=BASE_URL?>script/php/form_penilaian_perilaku_napi.php');
	});
				...
				success : function(data) {
					if(data.error == false){
			            ...
			            setTimeout(function () {
			                ...
			                ajaxloading('divList');
							$('#divForm').load('<?=BASE_URL?>script/php/form_penilaian_perilaku_napi.php');
							$('#divList').load('<?=BASE_URL?>script/php/list_penilaian_perilaku_napi.php');
			            }, timeout);
					}
					else{
...
if(!empty($_GET['id']) || $_GET['id'] != ''){
	$sql = "select nmKategori, nilaiBatasBawah, nilaiBatasAtas from p_range_perilaku_napi 
...
<input type="hidden" name="action" value="input_perilaku_napi">
<input type="hidden" name="isEdit" value="<?=$isEdit;?>">
<div class="center-block col-sm-5" style="padding-left:0px;">
	<div class="panel panel-info">
	<div class="panel-heading" align="left">
			<b class="panel-title">Input Range Penilaian Kecenderungan perilaku narapidana/tahanan</b>
		</div>
