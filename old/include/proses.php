...
switch($_POST['action']){
	case 'add_account' :
		if(empty($_POST['isEdit']) || $_POST['isEdit'] == ''){
			$sql = "select userid from p_user_management 
					where kd_propinsi = '".$_POST['kdPropinsi']."' and kd_kota = '".$_POST['kdKota']."'";
			...
			$sql = "insert into p_user_management (userid, passwd, username, email, kd_access, kd_propinsi, kd_kota,
					is_active, online_status, datecreate, resetcode)
					values ('".$_POST['userid']."', '".md5(md5("pa".$_POST['password']."ss"))."', '".$_POST['username']."', '".$_POST['email']."', '".$_POST['userAccess']."', '".$_POST['kdPropinsi']."', '".$_POST['kdKota']."', 
			...
			$sql = "update p_user_management set ".$password." username = '".$_POST['username']."', 
					...
					kd_kota = '".$_POST['kdKota']."',is_active = '".$_POST['isActive']."' where kd_user = '".$_POST['isEdit']."'";	
		}
		$exe = mysqli_query($connDB, $sql);
		writeLog(__LINE__, __FILE__, mysqli_error($connDB));
	...
	case 'upload_dokumen' :
		for($x=0;$x<$_POST['participants'];$x++){
			...
			$sql = "insert into p_dokumen_pendukung (nmFile, keterangan, 	isActive) 
					values ('".$fileUpload[$x]."', '".$_POST['keterangan'][$x]."', 1)";
			$exe = mysqli_query($connDB, $sql);
			writeLog(__LINE__, __FILE__, mysqli_error($connDB));
		}
	...
	case 'input_upt' :
		if($_POST['isEdit'] == ''){
			...
			$sql = "update kabupatenkota set kabupatenkotaname = '".$_POST['nmUpt']."', r_provinsiid = '".$_POST['wilayahID']."',
						alamat = '".$_POST['alamat']."', email = '".$_POST['email']."', nama_kepala = '".$_POST['nama_kepala']."', 
						no_telp_kepala = '".$_POST['no_telp_kepala']."'
...
switch($_GET['act']){
	...
	case 'getPropinsi' :
		if(intval($_GET['param']) == 0 && $_GET['param'] != ''){
			...
			$sql = "select provinsiid, provinsiname from provinsi order by provinsiid";
			$exe = mysqli_query($connDB, $sql);
			writeLog(__LINE__, __FILE__, mysqli_error($connDB));
			$i=0;
			while($row = mysqli_fetch_array($exe, MYSQLI_ASSOC)){
				$i++;
				$selected = ($row['provinsiid'] == $_GET['param']) ? "selected" : "";
