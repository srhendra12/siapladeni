...
<head>
	...
	function tutup(){
		var kdKriteria 	= $("#kd_kriteria").val();
		var sektor 		= $('#sektor').val();
		self.parent.ajaxloading('divList');
		self.parent.$('#divList').load('<?=BASE_URL?>script/php/list_evaluasi_pasca_konstruksi.php?kdKriteria=' + kdKriteria +'&sektor='+ sektor);
...
if(!empty($_GET['id']) || $_GET['id'] != ''){
	$sql = "select indikator, bobot, is_active from p_evaluasi_pasca_konstruksi
			where kd_pasca_konstruksi = ".$_GET['id'];
...
<input type="hidden" id="action" name="action" value="add_pasca_konstruksi">
<input type="hidden" id="isEdit" name="isEdit" value="<?=$_GET['id']?>">
<input type="hidden" id="kd_kriteria" name="kd_kriteria" value="<?=$_GET['kdKriteria']?>">
<input type="hidden" id="sektor" name="sektor" value="<?=$_GET['sektor']?>">
<div class="container-fluid" style="margin-top: 4px;">
    <div class="row">
        <div class="col-md-6 col-md-offset-4 col-center">
            <div class="panel panel-info" style="margin-bottom: 0px;">
                <div class="panel-heading">
                  <b class="panel-title">Tambah Indikator Penilaian Pasca Konstruksi</b>
                </div>
                <div class="panel-body">
					<table class="table table-striped table-condensed">
                    	<tbody>
                    		<tr>
                    			<td width="30%"><label class="control-label" for="indikator">Indikator</label></td>
                				<td><textarea class="form-control input-sm" rows="2" id="indikator" name="indikator"><?=$indikator?></textarea></td>
                    		</tr>
                    		<tr>
                    			<td><label class="control-label" for="bobot">Bobot</label></td>
