				...
				<div id="exTab3">
					...
					<div class="tab-pane" id="uploadDok">
						...
						<div class="col-md-6">
							<div class="panel panel-info">
								<div class="panel-heading">
									<b class="panel-title">Unduh Surat Pernyataan Keabsahan dokumen :</b>
								</div>
								<div class="panel-body">
									<?php
									$where = ($_SESSION['access'] < 4) ? "and kd_wilayah = ".$_SESSION['wilayah'] : "";
									$sql = "select nmFile, keterangan from p_dokumen_pendukung where isActive = 1 and jenis_dokumen = 1 ".$where;
									$exe = mysqli_query($connDB, $sql);
									writeLog(__LINE__, __FILE__, mysqli_error($connDB));
									echo "<ul class='list-group'>";
									while($row = mysqli_fetch_array($exe, MYSQLI_ASSOC)){
										echo "<li class='list-group-item'><a href='".BASE_URL."attachment/dokumen_pendukung/".$row['nmFile']."'>".$row['nmFile'].($row['keterangan'] != "" ? " - ".$row['keterangan'] : "")."</a></li>";
									}
									echo "</ul>";
									?>
								</div>
